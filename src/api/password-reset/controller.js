...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your First_Project account.<br>
        ...
        &mdash; First_Project Team
      `
      return sendMail({ toEmail: email, subject: 'First_Project - Password Reset', content })
    })
